...
from .conv import Conv, DWConv, GhostConv, LightConv, RepConv, autopad,DCNv4_mod
...
class Bottleneck_DCNv4(nn.Module):
    ...
    def __init__(self, c1, c2, shortcut=True, g=1, k=(3, 3), e=0.5):  # ch_in, ch_out, shortcut, groups, kernels, expand
        ...
        c_ = int(c2 * e)  # hidden channels
        self.cv1 = Conv(c1, c_, k[0], 1)
        self.cv2 = DCNv4_mod(c_, c2, k[1], 1, groups=g)
        self.add = shortcut and c1 == c2
...
class ResCCA(nn.Module):
    def __init__(self, in_channels, recurrence=1):
        super(ResCCA, self).__init__()
     
        self.recurrence = recurrence
        # self.cv1 = Conv(in_channels, inter_channels, 1, 1)
...
class ResCCAv2(nn.Module):
    def __init__(self, in_channels):
        super(ResCCAv2, self).__init__()
     
        # self.recurrence = recurrence
        # self.cv1 = Conv(in_channels, inter_channels, 1, 1)
        self.cca1 = CrissCrossAttention(in_channels)
        self.cca2 = CrissCrossAttention(in_channels)
